...
def build_divm_loader(cfg, mode='train', indices=None, probs=None, psl=None, return_idx=False):
    ...
    if mode == 'eval_train':
        ...
        label_cfg.ds_dict.return_idx = return_idx
        ...
        unlabel_cfg.ds_dict.return_idx = return_idx
        ...
        return test_loader
    elif mode == 'test_label_or_unlabel':
        test_lou_set_cfg = copy.deepcopy(cfg.data.test_label_or_unlabel)
        test_lou_set_cfg.ds_dict.indices = indices
        test_lou_set = build_dataset(test_lou_set_cfg)
        test_loader = DataLoader(
            test_lou_set, batch_size=cfg.batch_size,
            shuffle=False, num_workers=cfg.num_workers, drop_last=False
        )
        return test_loader
    elif mode == 'select':
        select_cfg = copy.deepcopy(cfg.data.select)
        select_cfg.ds_dict.indices = indices
        select_cfg.ds_dict.psl = psl
        select_cfg.ds_dict.return_idx = return_idx

        select_set = build_dataset(select_cfg)
        select_loader = DataLoader(
            select_set, batch_size=cfg.batch_size,
            shuffle=True, num_workers=cfg.num_workers, drop_last=False
        )
        return select_loader
    else:
        raise ValueError
